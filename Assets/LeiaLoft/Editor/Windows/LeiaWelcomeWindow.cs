...
namespace LeiaLoft.Editor
{
    ...
    [InitializeOnLoad]
    public class LeiaWelcomeWindow : EditorWindow
    {
        ...
        void InitUI()
        {
            ...
            sampleScenes.Elements.Add(new WelcomeUIElement(
                  "Multiple Camera",
                  "Multiple Camera Sample Scene",
                  "It is common practice to have two separate cameras: one to render the 3d scene, and another to render the UI on top of it. Multiple Camera demonstrates how to properly implement a multi-camera setup using the Leia Unity SDK.",
                  "Open Multiple Camera Scene",
                  () => { EditorSceneManager.OpenScene(string.Format("{0}{1}", examplesPath, "MultipleCameras/MultipleCameras.unity")); }, true));
        }
    }
}
